...
public class CameraViewModel extends ViewModel {
    ...
    private void getEuAngles(Mat rotMat){
        ...
        if ((-10.0 <= pitch.getValue() && pitch.getValue() <= 10.0) &&
                (-20.0 <= yaw.getValue() && yaw.getValue() <= 20.0)){

            capStateColor.postValue("#00ff00");
            toastTextForCaptureStatus.postValue("Front picture has been captured!!!");
            headPose.postValue("Front");

        }

        else if ((-10.0 <= pitch.getValue() && pitch.getValue() <= 10.0) &&
                (30.0 <= yaw.getValue() && yaw.getValue() <= 50.0)){

            capStateColor.postValue("#FFD700");
            toastTextForCaptureStatus.postValue("Left picture has been captured!!!");
            headPose.postValue("Left");
        }

        else if ((-10.0 <= pitch.getValue() && pitch.getValue() <= 10.0) &&
                (-50.0 <= yaw.getValue() && yaw.getValue() <= -30.0)){

            capStateColor.postValue("#00FFFF");
            toastTextForCaptureStatus.postValue("Right picture has been captured!!!");
            headPose.postValue("Right");
        }

        else if ((17.0 <= pitch.getValue() && pitch.getValue() <= 45.0) &&
                (-46.0 <= yaw.getValue() && yaw.getValue() <= 46.0) ){

            capStateColor.postValue("#FF00FF");
            toastTextForCaptureStatus.postValue("Random picture has been captured!!!");
            headPose.postValue("Random");
        }else {
